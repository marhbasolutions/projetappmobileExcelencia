...
const SinisteValideScreen = ({route,navigation}) => {
    ...
    const getSinistres = () => {
      ...
      fetch(API_GET_MY_SINISTRES+'?contrat='+contractNumber+'&done=1', {
          method: 'GET',
          headers: {
    ...
    {
      let icon = <Icon type='material-community' name='car-multiple' color='#f6d147' size={26} /> 
      if(String(item.type).toLowerCase() == 'vol'){
        icon = <Icon type='font-awesome' name='user-secret' color='#f6d147' size={26} /> 
        }
      else if(String(item.type).toLowerCase() == 'bris de glace'){
        icon = <Icon type='material-community' name='car-multiple' color='#f6d147' size={26} /> 
      }
      else
      {
        icon = <Icon type='material-community' name='fire-extinguisher' color='#f6d147' size={26} /> 
      }
            ...
            { !saving ? (
               sinistres.length == 0 ? (
                 ...
                 <View style={{flex:1,justifyContent:'center'}}>
                    <Text style={{textAlign:'center'}}>Pas de sinistre ...</Text>
                  </View>
...
export default SinisteValideScreen;
